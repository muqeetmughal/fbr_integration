import frappe
from fbr_integration.fbr_integration.api.fbr_api import get_fbr_api_config, handle_disabled_integration
import os
import tqdm
    ...
    for prov in tqdm.tqdm(provinces, desc="Inserting Provinces"):
        ...
        if province_name:
            try:
                ...
                doc = frappe.get_doc({
                    "doctype": "Buyer Province",
                    "buyer_province": province_name
                })
                doc.insert(ignore_if_duplicate=True)
            except Exception as e:
    ...
    for st in tqdm.tqdm(sale_types, desc="Inserting Sale Types"):
        ...
        if  sale_type_desc:
            try:
                ...
                frappe.log_error(f"Error inserting Sale Type {sale_type_desc}: {str(e)}")